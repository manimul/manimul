...
async function createData() {
  ...
  await client.delete({
    ...
    params: {
      types: ['home', 'project', 'artist', 'genre', 'sanity.imageAsset'],
    },
  });
  ...
  const projects: SanityDocumentLike[] = [];

  for (let postI = 0; postI < PROJECT_COUNT; postI++) {
  ...
  const data = [home, ...artists, ...genres, ...projects];
