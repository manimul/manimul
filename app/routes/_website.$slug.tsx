...
import { type ProjectDocument, projectZ } from '~/types/project';
...
// Perform a `like` or `dislike` mutation on a `project` document
...
// Load the `project` document with this slug
export const loader = async ({ params, request }: LoaderFunctionArgs) => {
  const { options } = await loadQueryOptions(request.headers);

  const query = PROJECT_QUERY;
  const initial = await loadQuery<ProjectDocument>(
    query,
    ...
    ...res,
    data: res.data ? projectZ.parse(res.data) : null,
  }));
